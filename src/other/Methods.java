...
public enum Methods {
	...
	public static int DTPX(int x,Methods m){
		if(m==East)return x+1;
		else if(m==Western)return x-1;
		else return x;
	}
	public static int DTPY(int y,Methods m){
		if(m==South)return y+1;
		else if(m==North)return y-1;
		else return y;
	}
	
	public static Player getPlayerFramArea (GameData data,int cx,int cy){
		Iterator<GameEntity> entitys = data.getUniverse().getGameEntitiesIterator();
	...
	public static boolean attackArea(GameData data,List<GameEntity>ignore,int x,int y){
		Iterator<GameEntity> entitys = data.getUniverse().getGameEntitiesIterator();
		Boolean res = true;
		for(GameEntity entity;entitys.hasNext();){
			entity=entitys.next();
	...
	public static void attackDirectionWithPower(Methods direction,int power,GameData data,List<GameEntity>ignore,int cx,int cy){
		if(power==0)return;
		if(attackArea(data, ignore, CTP(data, cx), CTP(data, cy)))attackDirectionWithPower(direction, power-1, data, ignore, DTPX(cx, direction), DTPY(cy,direction));
	}
	public static void explode(int power,GameData data,List<GameEntity>ignore,int x,int y){
		attackDirectionWithPower(Methods.East, power, data, ignore, PTC(data, x), PTC(data, y));
		attackDirectionWithPower(Methods.North, power, data, ignore, PTC(data, x), PTC(data, y));
		attackDirectionWithPower(Methods.South, power, data, ignore, PTC(data, x), PTC(data, y));
		attackDirectionWithPower(Methods.Western, power, data, ignore, PTC(data, x), PTC(data, y));
	}
	public static void explode(int power,GameData data,GameEntity ignore,int x,int y){
		LinkedList<GameEntity> tmp = new LinkedList<GameEntity> ();
		tmp.add(ignore);
